...
export default function LessonPage() {
  ...
  useEffect(() => {
    async function loadCourseLanguage() {
      try {
        const courseId = localStorage.getItem("praktikActiveCourseId");
        ...
        if (courseId) {
          ...
          const courseLanguage = data?.language;
          ...
          if (
            ...
            localStorage.setItem("praktikCourseLanguage", courseLanguage);
          }
        }
      } catch (error) {
        console.error("COURSE LANGUAGE ERROR:", error);
      } finally {
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="hidden rounded-full bg-slate-100 px-3 py-2 text-xs font-black uppercase text-slate-600 sm:inline-flex">
              {language === "az" ? "🇦🇿 AZ" : language === "en" ? "🇬🇧 EN" : "🇷🇺 RU"} 🔒
            </span>
